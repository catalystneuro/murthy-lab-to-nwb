...
class Cowley2022MappingBehaviorInterface(BaseDataInterface):
    ...
    def run_conversion(self, nwbfile: NWBFile, metadata: dict):
        ...
        # Add a processing module for storing songs
        ...
        self.pulse_times = sound_and_joints_data["pulse_times"]  # The timestamps of the pulse event
        self.pulse_types = sound_and_joints_data["pulse_types"]  # The type (0 is slow and 1 is fast)
        self.sine_present = sound_and_joints_data["sine_present"]  # A bolean array indicating presence of sine

        # Extract slow and fast pulses timestamps

        pslow_mask = self.pulse_types == 0
        pfast_mask = ~pslow_mask
        slow_pulse_timestamps = self.pulse_times[pslow_mask] / 10000  # Transform to seconds
        fast_pulse_timestamps = self.pulse_times[pfast_mask] / 10000  # Transform to seconds
        # pulse_times[-1] / (60 * 10000) gives ~ 30 minutes which is the stated recorded time.

        # Extract sine timestamps
        frames_where_sine_is_present = np.where(self.sine_present == 1)[0]
        sampling_frequency = 30.0  # From readme and paper
