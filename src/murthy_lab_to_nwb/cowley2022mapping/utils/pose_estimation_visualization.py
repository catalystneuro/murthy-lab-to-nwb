...
def extract_pose_estimation_data(pose_estimation_module: ProcessingModule) -> pd.DataFrame:
    ...
    data_frames_list = (df
                        .set_index(["track", "node"], append=True)
                        .unstack(level=[1, 2])
...
def generate_video_to_display(source_video_file_path, data_df, start_frame, end_frame, frame_offset):
    ...
    displayed_video = Video(data=in_memory_video.getvalue(), width=video_width, 
                            height=video_heigth, embed=True, mimetype="video/mp4")
...
class PoseEstimationWidgetController(widgets.VBox):
    ...
    def __init__(self):
        ...
        self.start_frame_widget = widgets.FloatText(
            value=0,
            description='Start time (s):',
            disabled=False
        )

        self.end_frame_widget = widgets.FloatText(
            value=1,
            description="End time (s)",
            disabled=False  
        )

        self.frame_offset_widget =  widgets.FloatText(
            value=0,
            description="Time offset (s)",
            disabled=False  
        )
...
class PoseEstimationWidget(widgets.HBox):
    ...
    def button_action(self, button_instance):
        ...
        with self.display_widget:
            display(generate_video_to_display(source_video_file_path=self.video_file_path, data_df=self.data_df,
                                            start_frame=start_frame, end_frame=end_frame, frame_offset=frame_offset))
